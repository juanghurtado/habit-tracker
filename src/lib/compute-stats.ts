...
function computeCurrentStreak(
  ...
  const earliestDate = createdAt
    ? new Date(
        Math.max(new Date(createdAt).getTime(), Date.now() - 365 * 86_400_000)
      )
  ...
  while (current >= earliest) {
    ...
    if (habitType === "good" && hasCompletion) {
      streak++;
    } else if (habitType === "bad" && !hasCompletion) {
      streak++;
    } else {
      break;
    }
...
function computeLongestStreak(
  ...
  const maxSpan = Math.min(
    366,
    Math.ceil((today.getTime() - firstDate.getTime()) / 86_400_000) + 1
  );
  if (maxSpan <= 0) {
    return 0;
  }
...
export function computeStats(
  ...
  const allHabitStats: HabitStats[] = habits.map((habit) => {
    const lifetimeTotal = completions.filter(
